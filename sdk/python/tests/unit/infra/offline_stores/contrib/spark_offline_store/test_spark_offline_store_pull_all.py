"""
Unit tests for SparkOfflineStore pull_all_from_table_or_query and
pull_latest_from_table_or_query SQL generation.
...
Also covers field_mapping alias behaviour when feature_name_columns=[].
"""
...
def test_pull_all_empty_feature_cols_with_field_mapping_adds_aliases(repo_config):
    """
    When feature_name_columns=[] and the data source has a field_mapping that
    renames a timestamp column, SELECT * must be augmented with the alias
    expression so downstream nodes (SparkFilterNode/SparkDedupNode) find the
    mapped (canonical) name in the resulting DataFrame.

    Feast field_mapping convention: keys are SOURCE column names, values are
    canonical names (e.g. {"src_ts": "event_timestamp"} means the source
    column is "src_ts" and the canonical name is "event_timestamp").
    The timestamp_field / join_key_columns parameters use SOURCE names.

    Expected SQL: SELECT *, src_ts AS event_timestamp FROM ...
    """
    source_with_mapping = SparkSource(
        name="mapped_reviews",
        path="s3a://bucket/processed/reviews/",
        file_format="parquet",
        # timestamp_field is the SOURCE column name
        timestamp_field="src_ts",
        field_mapping={"src_ts": "event_timestamp"},
    )
    mock_spark = MagicMock()
    with (
        patch(
        ...
        job = SparkOfflineStore.pull_all_from_table_or_query(
            config=repo_config,
            data_source=source_with_mapping,
            join_key_columns=["user_id"],
            feature_name_columns=[],
            # Pass the SOURCE column name, matching field_mapping convention
            timestamp_field="src_ts",
            created_timestamp_column=None,
            start_date=START,
            end_date=END,
    ...
    assert "SELECT *" in sql, f"Expected SELECT * in query, got: {sql[:120]!r}"
    assert "src_ts AS event_timestamp" in sql, (
        "field_mapping alias must appear alongside SELECT * so downstream "
        "nodes that reference 'event_timestamp' (canonical name) find it "
        f"in the DataFrame. Got SQL: {sql[:200]!r}"
    )


# ---------------------------------------------------------------------------
# pull_latest_from_table_or_query tests
# ---------------------------------------------------------------------------


def _run_pull_latest(repo_config, spark_source, feature_name_columns):
    """
    Call pull_latest_from_table_or_query with mocked SparkSession and
    data-source table resolution, then return the SQL query string.
    """
    mock_spark = MagicMock()
    with (
        ...
        job = SparkOfflineStore.pull_latest_from_table_or_query(
            config=repo_config,
            data_source=spark_source,
            join_key_columns=["user_id"],
            feature_name_columns=feature_name_columns,
...
def test_pull_latest_with_empty_feature_cols_generates_select_star(
    repo_config, spark_source
):
    """
    feature_name_columns=[] must produce SELECT * in the inner subquery so
    UDF-based BatchFeatureViews receive all raw source columns.
    """
    sql = _run_pull_latest(repo_config, spark_source, feature_name_columns=[])

    inner_select = sql.split("FROM (")[1].split("FROM")[0]
    assert "SELECT *" in inner_select, (
        "Expected 'SELECT *' in inner subquery when feature_name_columns=[], "
        f"got: {inner_select[:120]!r}"
    )


def test_pull_latest_with_feature_cols_generates_explicit_projection(
    repo_config, spark_source
):
    """
    When feature_name_columns is non-empty, query must use explicit projection.
    """
    sql = _run_pull_latest(
        repo_config,
        spark_source,
        feature_name_columns=["avg_rating", "review_count"],
    )
    ...
    assert "user_id" in sql
    assert "event_timestamp" in sql
    inner_before_from = sql.split("FROM (")[1].split("FROM")[0]
    assert "SELECT *" not in inner_before_from, (
        "Non-empty feature_name_columns must not produce SELECT * in inner query"
    )
