...
class SparkWriteNode(DAGNode):
    ...
    def execute(self, context: ExecutionContext) -> DAGValue:
        ...
        #
        # foreachPartition instead of mapInArrow: mapInArrow triggered
        # ArrowStreamPandasUDFSerializer (instead of ArrowStreamUDFSerializer)
        # for non-empty partitions when a WindowGroupLimitExec node was
        # immediately upstream in the plan, causing:
        #     AttributeError: 'list' object has no attribute 'dtype'
        # foreachPartition uses Python pickle serialization (no Arrow UDF
        # bridge) which avoids the issue entirely.
